...
import { authProviderId, MinimalModeError } from '../common/authentication';
...
// LOCAL MODE: Fake session that's always returned
const FAKE_SESSION: AuthenticationSession = {
	id: 'local-session',
...
async function getAuthSession(providerId: string, defaultScopes: string[], getSilentSession: () => Promise<AuthenticationSession | undefined>, options: AuthenticationGetSessionOptions = {}) {
	// LOCAL MODE: Always return fake session
	return FAKE_SESSION;
}
